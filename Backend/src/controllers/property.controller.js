...
const getPropertiesByUserId = async (req, res) => {
  try {
    ...
    const properties = await Property.find({ user_id }).populate('user_id', 'name email').lean();
    ...
    console.error('Error al obtener propiedades por user_id:', error);
    res.status(500).json({ message: 'Error al obtener propiedades por user_id', error });
  }
};
